...
def create_postgresql_tables(conn: Connection, ls_schema: str, cs_schema: str):
    ...
    create_area_tables(conn, ls_schema, cs_schema)
    create_crossing_tables(conn, ls_schema, cs_schema)

    print(
        f"Created PostgreSQL tables (trajectory_ls, stop_poly, area_poly, crossing_ls) in schema '{ls_schema}' and CellString tables (trajectory_cs, stop_cs, area_cs, crossing_cs) in schema '{cs_schema}'."
    )
