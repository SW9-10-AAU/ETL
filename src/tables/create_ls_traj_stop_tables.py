...
def create_ls_traj_stop_tables(conn: Connection):
    ...
    # Trajectory table
    cur.execute("""
            CREATE TABLE IF NOT EXISTS prototype2.trajectory_ls_new (
    ...
    cur.execute("""
            ...
            ON prototype2.trajectory_ls_new (mmsi);
        """)
    cur.execute("""
            CREATE INDEX IF NOT EXISTS trajectory_time_idx
            ON prototype2.trajectory_ls_new (ts_start, ts_end);
        """)
    cur.execute("""
            CREATE INDEX IF NOT EXISTS trajectory_geom_idx
            ON prototype2.trajectory_ls_new 
            USING GIST (geom);
        """)
    ...
    # Stop table
    cur.execute("""
            CREATE TABLE IF NOT EXISTS prototype2.stop_poly_new (
    ...
    cur.execute("""
            ...
            ON prototype2.stop_poly_new (mmsi);
        """)
    cur.execute("""
            CREATE INDEX IF NOT EXISTS stop_time_idx
            ON prototype2.stop_poly_new (ts_start, ts_end);
        """)
    cur.execute("""
            CREATE INDEX IF NOT EXISTS stop_geom_idx
            ON prototype2.stop_poly_new
            USING GIST (geom);
        """)
