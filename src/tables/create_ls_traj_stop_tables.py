...
def create_ls_traj_stop_tables(conn: Connection):
    ...
    # Schema
    cur.execute("""
            CREATE SCHEMA IF NOT EXISTS prototype1;
        """)

    # Trajectory table
    cur.execute("""
            CREATE TABLE IF NOT EXISTS prototype1.trajectory_ls_testing (
    ...
    cur.execute("""
            ...
            ON prototype1.trajectory_ls_testing (mmsi);
        """)
    cur.execute("""
            CREATE INDEX IF NOT EXISTS trajectory_time_idx
            ON prototype1.trajectory_ls_testing (ts_start, ts_end);
        """)
    cur.execute("""
            CREATE INDEX IF NOT EXISTS trajectory_geom_idx
            ON prototype1.trajectory_ls_testing 
            USING GIST (geom);
        """)
    ...
    # Stop table
    cur.execute("""
            CREATE TABLE IF NOT EXISTS prototype1.stop_poly_testing (
    ...
    cur.execute("""
            ...
            ON prototype1.stop_poly_testing (mmsi);
        """)
    cur.execute("""
            CREATE INDEX IF NOT EXISTS stop_time_idx
            ON prototype1.stop_poly_testing (ts_start, ts_end);
        """)
    cur.execute("""
            CREATE INDEX IF NOT EXISTS stop_geom_idx
            ON prototype1.stop_poly_testing 
            USING GIST (geom);
        """)
