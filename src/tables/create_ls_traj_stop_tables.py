...
def create_ls_traj_stop_tables(conn: Connection):
    ...
    # Trajectory table
    cur.execute("""
            CREATE TABLE IF NOT EXISTS prototype2.concave_trajectory_ls (
    ...
    cur.execute("""
            ...
            ON prototype2.concave_trajectory_ls (mmsi);
        """)
    cur.execute("""
            CREATE INDEX IF NOT EXISTS trajectory_time_idx
            ON prototype2.concave_trajectory_ls (ts_start, ts_end);
        """)
    cur.execute("""
            CREATE INDEX IF NOT EXISTS trajectory_geom_idx
            ON prototype2.concave_trajectory_ls 
            USING GIST (geom);
        """)
    ...
    # Stop table
    cur.execute("""
            CREATE TABLE IF NOT EXISTS prototype2.concave_stop_poly (
    ...
    cur.execute("""
            ...
            ON prototype2.concave_stop_poly (mmsi);
        """)
    cur.execute("""
            CREATE INDEX IF NOT EXISTS stop_time_idx
            ON prototype2.concave_stop_poly (ts_start, ts_end);
        """)
    cur.execute("""
            CREATE INDEX IF NOT EXISTS stop_geom_idx
            ON prototype2.concave_stop_poly
            USING GIST (geom);
        """)
