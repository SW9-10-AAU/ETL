...
def create_supercover_cs_traj_stop_tables(conn: Connection):
    cur = conn.cursor()
    ...
    # Stop table
    cur.execute("""
            CREATE TABLE IF NOT EXISTS prototype2.stop_supercover_cs
            (
        ...
        """)
    cur.execute("""
        CREATE INDEX IF NOT EXISTS stop_supercover_cs_mmsi_idx
        ON prototype2.stop_supercover_cs (mmsi);
    """)
    cur.execute("""
        CREATE INDEX IF NOT EXISTS stop_supercover_cs_time_idx
        ON prototype2.stop_supercover_cs (ts_start, ts_end);
    """)
    cur.execute("""
        CREATE INDEX IF NOT EXISTS stop_supercover_cs_z13_gin_idx
        ON prototype2.stop_supercover_cs
        USING GIN (cellstring_z13);
    """)
    cur.execute("""
        CREATE INDEX IF NOT EXISTS stop_supercover_cs_z21_gin_idx
        ON prototype2.stop_supercover_cs
        USING GIN (cellstring_z21);
    """)
    ...
    conn.commit()
    cur.close()
    print("Created Supercover CS trajectory and stop tables if not exist.")